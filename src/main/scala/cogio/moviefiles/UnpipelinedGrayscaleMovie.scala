...
/** Factory for creating a sensor that reads in successive frames of a
  ...
  * after it hits the last frame, forming an endless loop.  This uses an unpipelined
  * sensor, so the ffmpeg processing is done in series with any GPU processing of
  * the frames.
  *
  * @author Tobin Gonzalez and Dick carter
  */
object UnpipelinedGrayscaleMovie {
  ...
  /** Create a sensor (synchronous or fast-as-possible) that reads a
    ...
    * @return An UnpipelinedSensor that carries the movie's video stream.
    */
  def apply(filename: String, initTimestamp: Long, synchronous: Boolean, stride: Int): UnpipelinedSensor = {
    ...
    /** The UnpipelinedSensor behind this movie player- overrides hooks to enable saving of its state. */
    new UnpipelinedSensor(Shape(height, width), readFrame _, reset _, frameRate) {
      // The parameters that define the state of this sensor, packed into a string and separated by spaces.
      override def restoreParameters = Seq[Any](synchronous, grabber.getTimestamp, stride, filename).mkString(" ")

      // The default restoringClass object instance would identify this as an anonymous subclass of an UnpipelinedColorSensor.
      // We override this here to point to the UnpipelinedColorMovie factory object (so the restore method will be found).
      override def restoringClass = UnpipelinedGrayscaleMovie
    }
  }

  /** An alternate constructor that does not supply an initial timestamp and supplies some defaults for 'synchronous' and 'stride'. */
  def apply(filename: String, synchronous: Boolean = false, stride: Int = 1): UnpipelinedSensor = apply(filename, 0L, synchronous, stride)
  ...
  /** The factory method used to create an unpipelined sensor from its stored parameter string. */
  def restore(fieldType: FieldType, parameterString: String) = {
    //
    // The 4 parameters that define the state of the UnpipelinedMovie are:
    //
    ...
    // Suppose the filename has spaces in it?  By saving the filename last, we cover that case as follows:
