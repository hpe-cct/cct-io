...
object UnpipelinedColorMovie {
  ...
  /** Create a color sensor (synchronous or fast-as-possible) that reads a
    ...
    * @return An UnpipelinedColorSensor that carries the movie's video stream.
    */
  def apply(filename: String, initTimestamp: Long, synchronous: Boolean, stride: Int): UnpipelinedColorSensor = {
    ...
    /** The UnpipelinedColorSensor behind this movie player- overrides hooks to enable saving of its state. */
    new UnpipelinedColorSensor(Shape(height, width), readFrame _, reset _, frameRate) {
      ...
      override def restoringClass = UnpipelinedColorMovie
    }
  }

  /** An alternate constructor that does not supply an initial timestamp and supplies some defaults for 'synchronous' and 'stride'. */
  def apply(filename: String, synchronous: Boolean = false, stride: Int = 1): UnpipelinedColorSensor = apply(filename, 0L, synchronous, stride)
  ...
  def restore(fieldType: FieldType, parameterString: String) = {
    //
    // The 4 parameters that define the state of the UnpipelinedColorMovie are:
    //
